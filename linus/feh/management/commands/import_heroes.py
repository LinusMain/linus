...
from linus.feh.management.commands.curl_heroes import GetPklHeroURLFile, GetPklIconFile, GetPklOutputFile, GetPklDirectory
from linus.feh.models import AVAILABILITY, MOVEMENT_TYPE, WEAPON_TYPE, COLOR , F2P_LEVEL, \
  F2P_LEVEL_OVERRIDE
...
class Command(BaseCommand):
  ...
  def handle(self, *args, **options):
    ...
    all_data = GetKannaURLs(GetPklOutputFile(), GetPklIconFile(), GetPklHeroURLFile())
    ...
    for hero in heroes:
      ...
      for skillreq in hero.skillReqs:
        skills.append('{0}{1}'.format(skillreq.skill.name,
                                      skillreq.skill.wikiName))
    ...
    for skill in skills:
      if skill.isRefine or skill.isEnemyOnly or skill.slot == 'sacredseal':
        continue
      combin.append(('skill', skill, False,))
    ...
    for skilltype, skill, ismax in combin:
      ...
      weapon_perms = sorted(list(set([WEAPON_SLOT_MAP[WP_MAP[wp]] for wp in skill.weaponPerms])))
    ...
    for hero, raw_hero in hero_objs:
      for skillreq in raw_hero.skillReqs:
        ...
        skill_obj.heroes.append('{0}@{1}@{2}@{3}'.format(hero.f2p_level, hero.availability, '', hero))
