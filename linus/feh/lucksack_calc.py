...
def GetChances(orbs,
  ...
  def SimCircle(orbs, remain, opens, pitybroken, memo):
    ...
    for ((is_target, is_same_color, is_break_pity), raw_chance) in units_aggregate.items():
      if is_break_pity:
      ...
      if (orbs >= orb_costs[opens] and
        ...
        # open this circle.
        minires = SimCircle(orbs - orb_costs[opens],
                            remain-1,
                            opens+1,
...
def FehSnipeProbability(orbs, units, target_unit_id):
  ...
  for unit_id, color_id, is_break_pity, chance in units:
    mapkey = (unit_id == target_unit_id,
              color_id == target_color,
              is_break_pity)
    units_aggregate[mapkey] += chance

  memo = {}
  return GetChances(orbs,
                    0,
                    units_aggregate,
                    memo)
